...
// LoginChecker SegmentFault登录检查器
	...
	if strings.Contains(currentURL, "segmentfault.com/user/login") {
		...
		for {
			...
			case <-ticker.C:
				...
				// 检查是否已经跳转到首页（登录成功）
				if strings.Contains(currentURL, "segmentfault.com/") && !strings.Contains(currentURL, "/user/login") {
					...
					// 检查是否需要跳回原始页面（写作页面）
					if !strings.Contains(currentURL, "/write") {
						log.Printf("正在跳转回写作页面: %s", lc.originalURL)
						if _, err := page.Goto(lc.originalURL); err != nil {
...
func (lc *LoginChecker) publishArticles(page playwright.Page) {
	...
	log.Printf("准备发布 %d 篇文章到SegmentFault", len(lc.articles))
...
func IsLoginRequired(page playwright.Page) bool {
	currentURL := page.URL()
	return strings.Contains(currentURL, "segmentfault.com/user/login")
}